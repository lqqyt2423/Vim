...
- 第一次按`H`移至最上面，第二次按`H`往上翻半页
- 第一次按`L`移至最下面，第二次按`L`往下翻半页

### 按 ; , 上下一次可移动多行

可以修改配置`vim.moveMulitLines`设置一次移动的多行行数，默认为5。

当**没有**在当前行找到可移动的字符串时（按`f`搜索当前行），进行下面的操作：

- 按`;`向下移动5行，等同于`5j`
- 按`,`向上移动5行，等同于`5k`

如果修改配置`vim.moveMulitLinesMode`为`screen`，则上面的操作表现形式为移动屏幕。
